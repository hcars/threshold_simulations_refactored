...
function main()
    ...
    if !isfile(out_file_name)
        blocking_methods = ["no_block", "mcich", "mcich_org", "degree"]
    ...
    for i = 1:repetitions
		...
		for interaction_1 = 0:3
			...
			timing = string(end_time - start)
                        ...
                        # Find the original MCICH solution
                        blockers_smart_org = Blocking.mcich_org(
                            model,
                            seed_tup,
                            no_blocking_results,
                            selected_budgets,
                        ...
                        DiffusionModel.set_initial_conditions!(model, seed_tup)
                        DiffusionModel.set_blocking!(model, blockers_smart_org)


                        DiffusionModel.full_run(model)
                        blocking_summary_mcich_org = DiffusionModel.getStateSummary(model)
