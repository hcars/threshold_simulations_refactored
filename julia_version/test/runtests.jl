...
@testset "All Tests" begin
    ...
    @testset "Propogation Test: Low Threshold" begin
        ...
        model = DiffusionModel.MultiDiffusionModel(
            my_graph_1,
            node_states_1,
            thresholdStates_1,
            blockedDict_1,
    ...
    # Build graph
        ...
        model_other = DiffusionModel.MultiDiffusionModel(
            my_graph_2,
            node_states_2,
            thresholdStates_2,
            blockedDict_2,
        ...
        @testset "Iteration Tests" begin
            ...
            @test model_other.nodeStates == Dict{Int,UInt}([
                (2, 1),
                (3, 1),
                (1, 1),
                (4, 1),
        ...
        # Run and find optimal
            blocker = Blocking.mcich_optimal(
                model_other,
                (Set{Int}(), Set{Int}([2, 4, 5, 7])),
                full_run_2,
                [0, 1],
    ...
    @testset "Blocking Test: Binary Tree Graph" begin
        ...
        model_3 = DiffusionModel.MultiDiffusionModel(
            graph_3,
            node_states_3,
            thresholdStates_3,
            blockedDict_3,
        ...
        @testset "MCICH SMC Heuristic" begin
            ...
            DiffusionModel.set_initial_conditions!(
                model_3,
                (Set{Int}([1]), Set{Int}([1])),
            )
            ...
            @test sum(states_3[2:3]) +
                  2 * states_3[4] == sum(states_3_no_block[2:3]) +
                                     2 * states_3_no_block[4] - 14
        end
    ...
    @testset "Blocking Test: Star Graph" begin
        ...
        model_4 = DiffusionModel.MultiDiffusionModel(
            graph_4,
            node_states_4,
            thresholdStates_4,
            blockedDict_4,
    ...
    @testset "Hand Drawn Graph 1" begin
        ...
        model_5 = DiffusionModel.MultiDiffusionModel(
            graph_5,
            node_states_5,
            thresholdStates_5,
            blockedDict_5,
    ...
    @testset "Ascending Budget" begin
        ...
        model_6 = DiffusionModel.MultiDiffusionModel(
            graph_6,
            node_states_6,
            thresholdStates_6,
            blockedDict_6,
        ...
        @testset "MCICH SMC test on graph 6" begin
            ...
            for i = 1:50
                blocker = Blocking.mcich(
                    model_6,
                    (Set{Int}(collect(1:20)), Set{Int}(collect(1:20))),
                    full_run_6,
                    [i, 0],
                )[1]
                DiffusionModel.set_initial_conditions!(
    ...
    @testset "Simple Test for ILP construction" begin
        ...
        model_7 = DiffusionModel.MultiDiffusionModel(
            graph_7,
            node_states_7,
            thresholdStates_7,
            blockedDict_7,
        ...
        @testset "Check ILP construction" begin
            ilp = Blocking.ilp_construction(
                model_7,
                (Set{Int}([1]), Set{Int}([1])),
                full_run_7,
                1,
                GLPK.Optimizer,
                collect(Int, vertices(model_7.network)),
            )
    ...
    @testset "Jazz Net Test" begin
        ...
        node_states_8 = Dict()
        model_8 = DiffusionModel.MultiDiffusionModel(
            graph_8,
            node_states_8,
            thresholdStates_8,
            blockedDict_8,
                ...
                if curr_sum[2] + curr_sum[3] + curr_sum[4] - 20 != 0
                    ...
                    if active >= new_active
                        ...
                        if !isempty(previous_blockers[1])
                            if length(previous_blockers[1][1]) +
                               length(previous_blockers[1][2]) > length(blocker[1][1]) +
                                                                 length(blocker[1][2])
