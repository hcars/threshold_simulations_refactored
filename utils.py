...
def config_model(G, threshold, seed_set_1, seed_set_2, seed_set_3=None, blocked_1=[], blocked_2=[]):
    H = deepcopy(G)
    H.remove_nodes_from(blocked_1)
    H.remove_nodes_from(blocked_2)
    model = MultipleContagionThreshold(H)
    ...
    # Set threshold
    config.add_node_set_configuration('threshold_1', {u: threshold for u in H.nodes})
    config.add_node_set_configuration('threshold_2', {u: threshold for u in H.nodes})
    # Initialize seed set
